...
#the besy value of mtry is 9.
rf_mtry$bestTune$mtry
max(rf_mtry$results$Accuracy)
#because it has the maximum accuracy.
...
result_mtry <- resample(store_maxnode)
summary(result_mtry)
#the highest accuracy score is obtained with a value of maxnode equals to 39
#accuracy: 0.7914987, kappa: 0.5830748




#evaluate the model
finalmodel <- train(label~.,
                    data = rf_train,
                    method="rf",
                    metric="Accuracy",
                    tuneGrid=tuneGrid,
                    ...
                    maxnodes = 39,
                    ntree=100)
prediction <- predict(finalmodel, newdata = rf_test)
#confusionMatrix(prediction,rf_test$label)
